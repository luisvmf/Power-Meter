# Power-Meter
![alt tag](https://img.shields.io/badge/build-passing-brightgreen.svg)

Power Meter is a program developed to measure light intensity in experiments in undergraduate laboratories using an USB camera. The implementation of this program is described at (link to the article here).
    <p><img border="0" src="https://powermeter.luisvmf.com/screenshot.png" width="90%"></p>
To build for Linux use the scripts build-linux-32.sh for ia32 linux and build-linux-64.sh for x64 linux. You will need nodejs wget and g++ instaled on your system to use this script and a internet connection (the script will download a precompiled of electron:http://electron.atom.io.

To build the windows version follow the instructions on file "windows build instructions".

Download binary releases at:
<a href="http://powermeter.luisvmf.com" target="_blank">www.powermeter.luisvmf.com</a>


License: MIT
